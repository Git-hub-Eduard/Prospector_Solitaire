...
public class Layout : MonoBehaviour
{
    ...
    public Vector2 multiplier;//Смещение од центра раскладки
    //Ссылка SlotDef
    ...
    //Эта функция вызываетса для чтения файла LayoutXML.xml
    public void ReadLayout(string xmlText)
    {
        xmlr = new PT_XMLReader();
        xmlr.Parse(xmlText);// Загрузить xml
        xml = xmlr.xml["xml"][0];//Загрузить xml
        //Прочитать множители, определяющие растояние между картами
        ...
        //Прочитать слоты
        SlotDef tSD;
        //slotsX используетса для ускорения доступа к элементам <slot>
        ...
        for(int i = 0;i<slotsX.Count;i++)
        {
            ...
            //Преобразовать атрибуты в числовые значения 
            ...
            //Преобразовать номер ряда  в текст LayerName
